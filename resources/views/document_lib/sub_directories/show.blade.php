    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-2 offset-md-4">
                <a href="{{ route('dir_files.create', ['directory' => $directory->obfuscator]) }}" title="Upload Document" class="btn btn-outline-dark btn-block rounded-0" data-toggle="tooltip" data-placement="bottom" title="Upload Document">
                    <i class="fas fa-file-upload"></i>
                </a>
            </div>
            <div class="col-md-2">
                <a href="{{ route('dirs.create', ['directory' => $directory->obfuscator, 'in' => 'sub']) }}" class="btn btn-outline-secondary btn-block rounded-0" data-toggle="tooltip" data-placement="bottom" title="Create Directory">
                    <i class="fas fa-folder-open"></i>
                </a>
            </div>
            <div class="col-md-2">
                <a href="{{ route('dirs.edit', $directory->obfuscator) }}" class="btn btn-outline-success btn-block rounded-0" data-toggle="tooltip" data-placement="bottom" title="Edit Directory">
                    <i class="fas fa-edit"></i>
                </a>
            </div>
            <div class="col-md-2">
                <form method="POST" action="{{route('dirs.destroy', $directory->obfuscator)}}">
                    @method('DELETE')
                    {{ csrf_field() }}
                ...
                <div class="row">
                    @if (count($files) > 0)
                        @foreach ($files as $file)
                            <div class="col-md-3 mt-3">
                                <a href="{{ route('dir_files.show', $file->obfuscator) }}" class="no-text-decoration text-dark">
                                    <div class="card rounded-0">
                                        <div class="card-body p-2">
